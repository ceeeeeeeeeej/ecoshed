...
async function renderNotifications(skipStatUpdate = false) {
    ...
    container.innerHTML = displayNotifs.map(notif => {
        ...
        return `
            <div class="notification-item ${readClass} ${priorityClass}" data-id="${notif.id}" onclick="handleNotificationClick(event, '${notif.id}', ${notif.read}, '${notif.type}')">
...
window.handleNotificationClick = async function (event, id, isRead, type) {
    ...
    if (window.parent && typeof window.parent.navigateToPage === 'function') {
        ...
        } else if (type === 'special_collection' || type === 'pickup_request') {
