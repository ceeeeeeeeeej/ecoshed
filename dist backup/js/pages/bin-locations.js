...
function renderBinList(bins) {
    ...
    container.innerHTML = bins.map(bin => {
        ...
        const isFull = (bin.bin_status || '').toLowerCase() === 'full' || fillLevel >= 80;
                    ...
                    <div style="flex: 1;">
                        <div style="display: flex; justify-content: space-between; align-items: start;">
                            ...
                            <div style="text-align: right;">
                                ...
                                <div style="font-size: 10px; color: var(--text-muted);">${fillLevel}% Full</div>
                            </div>
                        </div>
    ...
    document.querySelectorAll('.bin-item').forEach(item => {
        item.addEventListener('click', () => {
            ...
            const marker = markersLayer.getLayers().find(m => m.getPopup()?.getContent()?.includes(id));
            if (marker) {
