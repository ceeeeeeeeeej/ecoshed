...
function updateSensorMetrics() {
    const now = new Date();
    // Online sensors: within 2 minutes of heartbeat
    const activeSensors = analyticsData.sensors.filter(s => {
        const lastUpdate = s.updated_at ? new Date(s.updated_at) : null;
        const diffMinutes = lastUpdate ? (now - lastUpdate) / 1000 / 60 : 999;
        return diffMinutes <= 2 && s.status !== 'inactive';
    });

    const criticalThreshold = 80;
    const lowBatteryThreshold = 20;

    // Critical bins: status is 'full' OR fill level >= 80%
    const criticalBins = analyticsData.sensors.filter(s =>
        (s.bin_status || '').toLowerCase() === 'full' ||
        (s.fill_level || 0) >= criticalThreshold
    );

    // 1. Average Fill Level Display: "Normal" vs "Full"
    const avgFillLevel = analyticsData.sensors.length > 0
        ? Math.round(analyticsData.sensors.reduce((sum, s) => sum + (s.fill_level || 0), 0) / analyticsData.sensors.length)
        : 0;

    const statusEl = document.getElementById('avgFillValue');
    if (statusEl) {
        statusEl.textContent = avgFillLevel >= criticalThreshold ? 'Full' : 'Normal';
        statusEl.style.color = avgFillLevel >= criticalThreshold ? '#ef4444' : '#10b981';
    }

    document.getElementById('avgFillChange').textContent = `Average density: ${avgFillLevel}% across ${analyticsData.sensors.length} bins`;
    ...
    document.getElementById('activeSensorsChange').textContent = `${activeSensors.length} sensors currently online`;
    ...
    // Broadcast Alert if there are new critical bins
...
window.navigateToBinsWithFilter = function (filterType) {
    ...
    if (window.parent && typeof window.parent.navigateToPage === 'function') {
        window.parent.navigateToPage('bins.html');
    } else {
    ...
    marker.bindPopup(`
        <div style="font-family: 'Inter', sans-serif; min-width: 200px; padding: 5px;">
            ...
            <div style="font-size: 12px; line-height: 1.5; color: #4b5563;">
                ...
                <p style="margin: 4px 0;"><strong>Fill Level:</strong> <span style="color: ${markerColor}; font-weight: bold;">${sensor.fill_level}%</span></p>
...
function showCriticalBinAlert(criticalBins) {
    ...
    const binList = criticalBins.map(b => `${b.bin_id} [${b.zone || 'Unknown Barangay'}] (${b.fill_level}%)`).join(', ');
...
async function storeNotification(title, message, priority, targetItems, alertType = 'iot') {
    ...
    try {
        ...
        // 1. Persist general notification for Admin Center (Admin only)
        if (dbService && dbService.createNotification) {
            await dbService.createNotification({
                title: title,
                message: message,
                type: 'bin_alert', // Changed from 'iot' to 'bin_alert' to trigger resident filters
                priority: priority
            });
        }

        // 2. Targeted Push to ALL Collectors
        ...
        if (users && users.length > 0) {
            ...
            for (const collector of users) {
                await dbService.createNotification({
                    ...
                    type: 'bin_alert', // Changed from 'iot' to 'bin_alert'
                    priority: 'high'
                });
            }
        }
...
/**
 * Applies a historical plan back to the Heatmap view using its array index.
 */
...
/**
 * Backward compatible wrapper for applyHistoricalPlan (ID-based)
 */
...
/**
 * Refreshes the plan history grid.
 */
