...
async function notifyCollectorsOfCriticalBin(bin) {
    try {
        ...
        const targetAreas = ['victoria', 'dayoan', 'dayaon']; // User priority areas
        const binArea = targetAreas.find(a => area.includes(a)) || 'all';

        const collectorsToNotify = users.filter(u => {
            const userArea = (u.barangay || '').toLowerCase();
            const userStatus = (u.status || '').toLowerCase();
            const isOperational = ['active', 'available', 'online', 'offline', 'scheduled'].includes(userStatus);
            
            // Notify if in the same area OR if user handles all areas
            // OR if it's one of the priority areas (broadcast to all victoria/dayoan collectors)
            if (!isOperational) return false;
            if (userArea === 'all') return true;
            if (binArea !== 'all' && (userArea.includes(binArea) || binArea.includes(userArea))) return true;
            
            return false;
        });

        console.log(`📣 Notifying ${collectorsToNotify.length} collectors for bin ${binId}`);
        ...
        for (const collector of collectorsToNotify) {
