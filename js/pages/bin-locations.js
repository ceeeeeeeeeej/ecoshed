...
function renderBinList(bins) {
    ...
    container.innerHTML = bins.map(bin => {
        ...
        const statusIndicator = isOnline ? `<span style="color: #10b981; font-size: 11px; font-weight: 600;"><i class="fas fa-circle" style="font-size: 8px; vertical-align: middle; margin-right: 4px;"></i>Online</span>` : `<span style="color: #9ca3af; font-size: 11px; font-weight: 600;"><i class="fas fa-circle" style="font-size: 8px; vertical-align: middle; margin-right: 4px;"></i>Offline</span>`;

        // Fill level status
        const fillLevel = bin.fill_level || 0;
        const isFull = (bin.bin_status || '').toLowerCase() === 'full' || fillLevel >= 75;
        const statusColor = isFull ? '#ef4444' : '#10b981';
        const statusLabel = isFull ? 'Full' : 'Normal';
                    ...
                    <div style="flex: 1;">
                        <div style="display: flex; justify-content: space-between; align-items: start;">
                            ...
                            <div style="text-align: right;">
                                ...
                                <div style="font-size: 10px; color: var(--text-muted);">Bin Status</div>
                            </div>
                        </div>
...
function renderMapMarkers(bins) {
    ...
    bins.forEach(bin => {
        ...
        if (currentCoord.lat && currentCoord.lng && parseFloat(currentCoord.lat) !== 0) {
            ...
            marker.bindPopup(`<strong>${binId}</strong><br>Location: ${bin.address || 'N/A'}<br><span style="font-size: 12px; color: #666;">Drag pin to move</span>`);
            marker.on('dragend', (e) => handlePositionUpdate(binId, e.target.getLatLng().lat, e.target.getLatLng().lng));
...
function setupEventListeners() {
    ...
    document.getElementById('binSearchInput').addEventListener('input', () => {
